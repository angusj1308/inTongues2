...
const ActiveMode = ({
  ...
  const showStickyControls = activeStep <= 3

  return (
    <div className={`active-flow active-step-${activeStep} ${showStickyControls ? 'has-pass-sticky' : ''}`}>
      <>
        ...
        {activeStep <= 3 && (
          <>
            {showStickyControls && (
              <div className="active-pass-sticky">
                <div className="active-pass-sticky-inner">
                  {renderProgressBar()}
                  {passNavigation}
                </div>
              </div>
