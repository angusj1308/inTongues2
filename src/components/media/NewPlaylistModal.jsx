...
// Surface-agnostic playlist creation modal. The caller injects `onCreate`
// (e.g. (uid, {name, description}) => createPlaylist(...)) so the same modal
// works for podcast and music playlists.
const NewPlaylistModal = ({
  ...
  const handleSubmit = async (event) => {
    ...
    setError('')
    try {
      const id = await onCreate(uid, { name, description })
      onCreated?.(id)
      onClose?.()
    } catch (err) {
      console.error(err)
