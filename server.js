 ...
 * @param {boolean} testMode - When true, logs full prompt/response under a banner
 * @returns {Promise<{chapterHeader: string, chapterOutline: string, bodyText: string}>}
 */
async function parseChapterStructure(chapterText, testMode = false) {
  ...
  try {
    ...
    const systemMessage = CHAPTER_STRUCTURE_PROMPT
    const userMessage = `Analyze the structure of this chapter text:\n\n${sampleText}`

    if (testMode) {
      const bar = '═'.repeat(60)
      console.log(`\n${bar}\nAI CALL — parseChapterStructure\n${bar}`)
      console.log('Model: gpt-4.1-mini')
      console.log('Parameters: (none beyond model + input)')
      console.log(`Input chapterText length: ${chapterText.length}`)
      console.log(`sampleText length sent (truncated to 2000): ${sampleText.length}`)
    ...
    if (!jsonMatch) {
      ...
      if (testMode) {
        console.log('\n--- PARSED RESULT ---')
        console.log('(non-JSON response; falling back to { chapterHeader: "", chapterOutline: "", bodyText: <full input> })')
      }
...
// --- Podcast search (Spotify + iTunes RSS resolution) ---------------------
    ...
    const data = await cached(
      ...
      async () => {
        ...
        if (parsed.parseError) {
          return {
            available: false,
            unavailableReason: 'feed_unreachable',
            episodes: [],
            nextCursor: null,
          }
        }
        return { available: true, episodes: parsed.episodes, nextCursor: null }
      },
    )
    if (!data) return res.status(404).json({ error: 'Show not found' })
...
// ============================================================
// YouTube captions via yt-dlp (replaces the HTML scraper + Whisper fallback)
// ============================================================
//
// The old fetchYoutubeCaptionSegments scrapes the public watch page and parses
// `ytInitialPlayerResponse` JSON. YouTube has progressively (a) stripped
// captions metadata from the public page for unauthenticated requests and
// (b) required a `pot` proof-of-origin token on direct caption-URL fetches —
// so the scrape now returns empty captionTracks or empty XML for almost every
// video, falls through to Whisper, and we get the comma-only blob output
// that triggered this rewrite.
//
// yt-dlp talks to YouTube's INNERTUBE player APIs (Android VR / iOS / Web
// Embedded clients) which still get caption metadata reliably. We probe
// metadata once with `-j` to confirm the video's original language and
// available caption tracks, then download SRT in that original language only.
...
// caption stream can't accumulate into a blob (the old failure mode).
function buildSentenceSegmentsFromCues(cues) {
  ...
  for (const c of cues) {
    ...
    const wordCount = (cur.text.match(/\S+/g) || []).length
    if (/[.!?…]$/.test(cur.text) || wordCount >= MAX_SEGMENT_WORDS) {
      out.push(cur)
      cur = null
    }
  }
...
function runYtDlp(args, { timeoutMs = 60000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      clearTimeout(timer)
      if (code !== 0) {
        return reject(new Error(`yt-dlp exited ${code}: ${stderr.slice(0, 500) || stdout.slice(0, 500)}`))
      }
...
/**
 * Pull captions for a YouTube video via yt-dlp, ALWAYS in the video's
 * original language (never auto-translated). Returns:
 *   { segments, sentenceSegments, text, originalLanguage, source } | null
 * `source` is 'manual' or 'asr' depending on whether the captions were
 * uploaded by the channel or auto-generated by YouTube.
 *
 * Throws on hard failures (yt-dlp not installed, network error, etc.).
 * Returns null when the video genuinely has no captions in its original
 * language — caller should mark the video as failed with a user-facing
 * "no captions available" error rather than retrying anywhere.
 */
async function fetchYoutubeCaptionsViaDlp(videoId, requestedLanguage = null) {
  const url = `https://www.youtube.com/watch?v=${videoId}`
  const tmpDir = path.join(os.tmpdir(), `yt-${videoId}-${Date.now()}`)
  await fs.mkdir(tmpDir, { recursive: true })

  try {
    // Step 1: probe metadata. Tells us the original language and which
    // caption tracks are manual vs ASR vs auto-translated.
    const { stdout } = await runYtDlp(['-j', '--skip-download', url], { timeoutMs: 45000 })
    let metadata = {}
    try {
      metadata = JSON.parse(stdout)
    } catch (parseErr) {
      throw new Error(`yt-dlp metadata JSON parse failed: ${parseErr.message}`)
    }

    const originalLang = (metadata.language || '').toLowerCase().trim()
    const subtitles = metadata.subtitles || {}
    const autoCaptions = metadata.automatic_captions || {}

    // Determine which language to pull.
    // Hard rule: prefer the video's actual original language. If yt-dlp
    // didn't report one, fall back to whatever the caller requested.
    const pullLang = originalLang || (requestedLanguage || '').toLowerCase().trim() || null
    if (!pullLang) {
    ...
    for (const c of candidates) {
      if (subtitles[c]) { chosenLang = c; source = 'manual'; break }
    }
    if (!chosenLang) {
      for (const c of candidates) {
    ...
    const sentenceSegments = buildSentenceSegmentsFromCues(cues)
    const text = sentenceSegments.map((s) => s.text).join(' ')

    return {
      segments: cues,
      sentenceSegments,
      text,
      originalLanguage: originalLang || pullLang,
      source,
    }
  } finally {
...
app.post('/api/youtube/transcript', async (req, res) => {
  ...
  try {
    ...
    // yt-dlp ALWAYS — original-language captions only, no Whisper fallback.
    let dlpResult
    try {
      dlpResult = await fetchYoutubeCaptionsViaDlp(videoId, languageCode)
    } catch (dlpError) {
      console.error('yt-dlp failed:', dlpError)
      return res.status(500).json({ error: dlpError?.message || 'Failed to fetch captions' })
    }
    if (!dlpResult) {
      return res.status(404).json({ error: 'No captions available for this video in its original language' })
    }

    const normalisedSegments = normaliseTranscriptSegments(dlpResult.segments)
    ...
    const transcriptPayload = {
      ...
      originalLanguage: dlpResult.originalLanguage,
      captionSource: dlpResult.source,
...
app.post('/api/transcribe', async (req, res) => {
  ...
  try {
    ...
    // yt-dlp ALWAYS — original-language captions only.
    let dlpResult
    try {
      dlpResult = await fetchYoutubeCaptionsViaDlp(videoId, 'en')
    } catch (dlpErr) {
      console.error('yt-dlp failed:', dlpErr)
      return res.status(500).json({ error: dlpErr?.message || 'Failed to fetch captions' })
    }
    if (!dlpResult) {
      return res.status(404).json({ error: 'No captions available for this video in its original language' })
    }
...
app.post('/api/transcribe/background', async (req, res) => {
  ...
  try {
    ...
    // yt-dlp captions in the video's original language. No Whisper.
    let segments = []
    try {
      console.log('Fetching captions via yt-dlp...')
      const dlpResult = await fetchYoutubeCaptionsViaDlp(videoId, 'en')
      if (!dlpResult) {
        await lessonRef.update({
          status: 'import_failed',
          importError: 'No captions available for this video in its original language',
        })
        return
      }
      segments = dlpResult.segments
      console.log(`yt-dlp captions complete: ${segments.length} segments (originalLanguage=${dlpResult.originalLanguage}, source=${dlpResult.source})`)
    } catch (dlpError) {
      console.error('yt-dlp captions failed:', dlpError.message)
      await lessonRef.update({
        status: 'import_failed',
        importError: `Caption fetch failed: ${dlpError.message}`,
      })
      return
    }
...
async function processYouTubeTranscript(uid, videoDocId, videoId, languageCode = 'auto') {
  ...
  try {
    ...
    // yt-dlp ALWAYS — original-language captions, no Whisper fallback.
    // If the video has no captions in its native language, fail the import
    // with a clear user-facing message rather than producing a translated
    // or transcribed approximation that might mismatch the spoken audio.
    const dlpResult = await fetchYoutubeCaptionsViaDlp(videoId, languageCode)
    if (!dlpResult) {
      const message = 'No captions available for this video in its original language'
      console.warn(`[yt-dlp ${videoId}] ${message}`)
      await videoRef.update({
        status: 'failed',
        audioError: message,
      }).catch(() => {})
    ...
    await videoRef.update({
      status: 'failed',
      audioError: error?.message || 'Failed to import video',
    }).catch(() => {})
    throw error
  }
}
...
app.get('/api/youtube/search', async (req, res) => {
  ...
  try {
    const data = await cached(cacheTag, 60 * 60, async () => {
      ...
      apiUrl.searchParams.set('type', 'video,channel,playlist')
      ...
      // Video counts for playlist results aren't in search.list — fetch
      // them via playlists.list keyed by all playlist IDs in this batch.
      ...
      if (playlistIds.length > 0) {
        ...
        plUrl.searchParams.set('id', playlistIds.join(','))
        ...
        try {
          ...
          if (plRes.ok) {
            ...
            ;(plData.items || []).forEach((p) => {
              if (!p.id) return
              playlistMeta.set(p.id, {
                videoCount: Number(p.contentDetails?.itemCount) || null,
              })
            })
          }
        } catch (err) {
          console.warn('playlists.list metadata lookup failed', err)
        }
      }
      ...
      const results = items
        .map((item) => {
          ...
          if (item.id?.playlistId) {
            ...
            const pMeta = playlistMeta.get(playlistId) || {}
            return {
              kind: 'playlist',
              playlistId,
              title: snippet.title || '',
              ...
              videoCount: pMeta.videoCount || null,
            }
          }
          return null
        })
...
app.get('/api/youtube/channel/:channelId/videos', async (req, res) => {
  ...
  const max = Math.max(1, Math.min(parseInt(req.query.max || '24', 10) || 24, 50))
  try {
    const data = await cached(
      ...
      async () => {
        ...
        const pUrl = new URL('https://www.googleapis.com/youtube/v3/playlistItems')
        pUrl.searchParams.set('part', 'snippet,contentDetails')
        pUrl.searchParams.set('playlistId', uploads)
        pUrl.searchParams.set('maxResults', String(max))
        if (cursor) pUrl.searchParams.set('pageToken', cursor)
        pUrl.searchParams.set('key', YOUTUBE_API_KEY)
        const pRes = await fetch(pUrl.toString())
        if (!pRes.ok) {
          const body = await pRes.text().catch(() => '')
          throw new Error(`playlistItems.list failed: ${pRes.status} ${body.slice(0, 200)}`)
        }
        const pPayload = await pRes.json()
        const items = pPayload.items || []
        const videoIds = items
          .map((it) => it.contentDetails?.videoId || null)
          .filter(Boolean)

        // Batch videos.list to get durations + language fields (snippet on
        // playlistItems doesn't include them).
        ...
        if (videoIds.length > 0) {
          ...
          try {
            ...
            console.warn('videos.list metadata lookup failed (channel videos)', err)
          }
        }
        ...
        const videos = items.map((it) => {
          ...
          return {
            ...
            publishedAt: it.contentDetails?.videoPublishedAt || snip.publishedAt || '',
            thumbnailUrl: thumb || (videoId ? `https://img.youtube.com/vi/${videoId}/mqdefault.jpg` : ''),
        ...
        return {
          videos,
          nextCursor: pPayload.nextPageToken || null,
          creditsPerMinute: ELEVENLABS_CREDITS_PER_MINUTE,
        }
      },
    )
    res.json(data)
    ...
    res.status(502).json({ error: 'YouTube channel videos lookup failed' })
  }
})

// Playlist metadata: { id, title, channelId, channelTitle, coverUrl,
// description, videoCount }. Cached for 1h since playlist titles/covers
// rarely change. Video list lives behind a separate endpoint so the
// metadata fetch is cheap.
app.get('/api/youtube/playlist/:playlistId', async (req, res) => {
  ...
  try {
    const data = await cached(
      cacheKey(['youtube-playlist', 'meta', playlistId]),
      60 * 60,
      async () => {
        ...
        return {
          ...
          coverUrl: thumbs.high?.url || thumbs.medium?.url || thumbs.default?.url || '',
          publishedAt: item.snippet?.publishedAt || '',
          videoCount: Number(item.contentDetails?.itemCount) || null,
        }
      },
    )
    ...
    console.error('YouTube playlist meta error', err)
    res.status(502).json({ error: 'YouTube playlist lookup failed' })
  }
})

// Paginated video listing for a playlist. Mirrors the channel-videos
// endpoint shape so the front-end can render either source identically.
app.get('/api/youtube/playlist/:playlistId/videos', async (req, res) => {
  ...
  try {
    const data = await cached(
      cacheKey(['youtube-playlist', 'videos', playlistId, String(max), cursor || 'first']),
      30 * 60,
      async () => {
        const pUrl = new URL('https://www.googleapis.com/youtube/v3/playlistItems')
        pUrl.searchParams.set('part', 'snippet,contentDetails')
        pUrl.searchParams.set('playlistId', playlistId)
        pUrl.searchParams.set('maxResults', String(max))
        if (cursor) pUrl.searchParams.set('pageToken', cursor)
        pUrl.searchParams.set('key', YOUTUBE_API_KEY)
        const pRes = await fetch(pUrl.toString())
        ...
        if (videoIds.length > 0) {
          ...
          try {
            ...
            console.warn('videos.list metadata lookup failed (playlist videos)', err)
          }
        }
        ...
        const videos = items.map((it) => {
          ...
          return {
            ...
            title,
            description,
            channelId: snip.videoOwnerChannelId || snip.channelId || '',
    ...
    res.status(502).json({ error: 'YouTube playlist videos lookup failed' })
  }
})
 ...
 */
function detectChapterMarkers(text, testMode = false) {
  ...
  if (testMode) {
    const bar = '═'.repeat(60)
    console.log(`\n${bar}\nCHAPTER DETECTION — CANDIDATES\n${bar}`)
    console.log(`Total lines in input: ${lines.length}`)
    console.log(`Total candidate lines found: ${candidates.length}`)
    console.log('\n--- CANDIDATES BY SIGNATURE GROUP ---')
    const signatures = Object.keys(signatureGroups)
    if (signatures.length === 0) {
      console.log('(no candidates)')
    } else {
      for (const signature of signatures) {
        const matches = signatureGroups[signature]
        console.log(`\nSignature: "${signature}"  (matches: ${matches.length})`)
        for (const m of matches) {
          console.log(`  lineIndex=${m.lineIndex}  position=${m.position.toFixed(4)}  line="${m.line}"`)
        }
      }
    }
    console.log('\n--- SCORED SIGNATURES (count >= 2) ---')
    let scoredAny = false
    for (const [signature, matches] of Object.entries(signatureGroups)) {
      ...
      const score = matches.length * (spread > 0.5 ? 2 : 1)
      console.log(`  "${signature}"  count=${matches.length}  min=${minPos.toFixed(4)}  max=${maxPos.toFixed(4)}  spread=${spread.toFixed(4)}  score=${score}`)
    }
    if (!scoredAny) {
      console.log('  (no signature had >= 2 matches)')
    }
    if (!bestPattern) {
      console.log('\nWinner: (no pattern detected — returning null)')
    } else {
      console.log(`\nWinner: "${bestPattern.signature}"  score=${bestScore}  chapters=${bestPattern.matches.length}`)
    }
  }
 ...
 */
async function extractTxtWithChapters(filePath, testMode = false) {
...
// Used exclusively by /api/import-upload when testMode=true.
// All calls to these are gated by `if (testMode)` at the call site
// so production imports produce no extra output.
// ============================================================
const TEST_MODE_BAR = '═'.repeat(60)
function tmBanner(label) {
  console.log(`\n${TEST_MODE_BAR}\n${label}\n${TEST_MODE_BAR}`)
}
function tmBlock(label, content) {
  tmBanner(label)
  console.log(content)
}
function tmKv(obj) {
...
app.post('/api/import-upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (testMode) {
      tmBanner('TEST MODE — FILE RECEIVED')
      tmKv({
    ...
    if (fileType === 'txt') {
      // Test-mode format diagnostics: read raw file to measure pre/post strip state.
      // This is a duplicate read, only in test mode — the real extractTxtWithChapters
      // still runs below unchanged.
      let txtDiag = null
      if (testMode) {
        const rawTxt = await fs.readFile(req.file.path, 'utf8')
        const rtfDetected = rawTxt.startsWith('{\\rtf')
        const afterRtf = stripRtfMarkup(rawTxt)
        const gutenbergMarkers = [
          '*** START OF THE PROJECT GUTENBERG EBOOK',
          '*** START OF THIS PROJECT GUTENBERG EBOOK',
          '***START OF THE PROJECT GUTENBERG EBOOK',
        ]
        const gutenbergDetected = gutenbergMarkers.some((m) => afterRtf.includes(m))
        const afterGutenberg = stripGutenbergBoilerplate(afterRtf)
        txtDiag = {
          rawLength: rawTxt.length,
          rtfDetected,
          afterRtfLength: afterRtf.length,
          gutenbergDetected,
          afterGutenbergLength: afterGutenberg.length,
        }
      }
      ...
      if (testMode) {
        ...
        tmBanner('FORMAT DIAGNOSTICS')
        tmKv({
          raw_file_length: txtDiag.rawLength,
          rtf_stripped: txtDiag.rtfDetected ? 'yes' : 'no',
          length_after_rtf_strip: txtDiag.afterRtfLength,
          gutenberg_stripped: txtDiag.gutenbergDetected ? 'yes' : 'no',
          length_after_gutenberg_strip: txtDiag.afterGutenbergLength,
        })

        tmBanner('CHAPTER DETECTION — INPUT')
        console.log(`Text char count passed to detector: ${txtDiag.afterGutenbergLength}`)
      ...
      if (testMode) {
        ...
        if (title || author) {
          ...
          console.log('source: (none — no title/author provided)')
          console.log('result_url: no cover found')
        }
      }
    ...
    if (fileType === 'pdf') {
      ...
      if (testMode) {
        ...
        const pdfRaw = await pdfParse(pdfBuf)
        const rawText = pdfRaw.text || ''
        const trimmed = rawText.trim()
        pdfDiag = {
          file_bytes: pdfBuf.length,
          pdf_parse_char_count: rawText.length,
          trimmed_char_count: trimmed.length,
          scanned_pdf_check: trimmed.length >= 100 ? 'pass (>= 100 chars)' : 'fail (< 100 chars — would throw SCANNED_PDF_NOT_SUPPORTED)',
        }
      }
    ...
    if (testMode) {
      tmBanner(`TEST MODE — ERROR IN ${failedStep}`)
...
app.post('/api/generate/concept', async (req, res) => {
  try {
    ...
    const prompt = trimmedFormat === 'short story'
      ? `You are ${authorName.trim()}. Create an original concept for a compelling ${genreQualifier}short story set in ${settingText}. Begin with the premise, a description of each main character followed by the plot outline detailed scene by scene. Each scene outline should contain only what happens where, who is present, what they do, what they say, and what changes. DO NOT write prose, suggest metaphors, or provide thematic commentary. DO NOT create plots or characters driven by progressive and woke ideological frameworks.\nRemember - This is only a SHORT story of approximately 5,000-10,000 words or 15-25 pages.\nEvery scene must move the plot forward in a meaningful way. DO NOT create separate scenes for continuous action that carries no new information, decision, or conflict.\nBegin your response with the title on its own line in the format:\nTitle: <title of the work>\nDeliver your concept as a continuous plot outline. Do not use headers, bullet points, or bold formatting.`
...
${concept.trim()}`
    ...
    // ── Validation call: check concept + summaries for inconsistencies ──
    console.log('REGENERATE CALL 2.5 — VALIDATING OUTLINE CONSISTENCY…')
    console.log('───────────────────────────────────────────────────────')
...
${summariesText}`
    ...
    console.log('REGENERATE CALL 2.5 — VALIDATION RESULT:')
    console.log('───────────────────────────────────────────────────────')
    console.log(validationText)
    console.log('═══════════════════════════════════════════════════════\n')
