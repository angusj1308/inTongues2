...
app.post('/api/generate/story/style-rewrite', async (req, res) => {
  try {
    const { uid, storyId, authorName } = req.body

    if (!uid?.trim()) return res.status(400).json({ error: 'uid is required' })
    if (!storyId?.trim()) return res.status(400).json({ error: 'storyId is required' })
    if (!authorName?.trim()) return res.status(400).json({ error: 'authorName is required' })

    // Fetch the Phase 3 prose from Firestore
    const storyRef = admin.firestore().collection('users').doc(uid).collection('stories').doc(storyId)
...
${phase3Prose.trim()}`
    ...
    if (!rewrittenText) {
      return res.status(500).json({ error: 'No rewritten text was generated.' })
    }
    ...
    return res.json({ success: true, storyText: rewrittenText, wordCount, storyId })
  } catch (error) {
    console.error('Style rewrite error:', error)
    return res.status(500).json({ error: 'Failed to style rewrite', details: error.message })
  }
})
