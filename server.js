...
app.post('/api/generate/prompt', async (req, res) => {
  try {
    ...
    const { concept: prompt, tensionText, tropeId } = await expandVagueConcept('from-scratch', librarySummaries)

    return res.json({ success: true, prompt, tensionText, tropeId })
  } catch (error) {
...
app.post('/api/generate/expand-prompt', async (req, res) => {
  try {
    ...
    const { concept: prompt, tensionText, tropeId } = await expandVagueConcept(concept.trim(), librarySummaries)
...
app.post('/api/generate/different-prompt', async (req, res) => {
  try {
    ...
    const { concept: prompt, tensionText, tropeId } = await generateDifferentConcept(existingConcept.trim(), librarySummaries)
